...
def get_model(model_name, adapted_component, adaptor_class, num_steers, rank,
    ...
    if model_name.startswith("EleutherAI/gpt-neo") or \
        ...
        return model, model.tokenizer
    # gpt2中文模型
    elif model_name.startswith("uer/gpt2"):
        from lm_steer.models.model_gpt_zh import Switching_GPT_zh_Model
        model = Switching_GPT_zh_Model(
            model_name, adapted_component, adaptor_class, num_steers, rank,
            epsilon, init_var, low_resource_mode)
        return model, model.tokenizer
    # Qwen 1.5B Instruct
    elif model_name.startswith("Qwen"):
        from lm_steer.models.model_qwen import Switching_Qwen_Model
        model = Switching_Qwen_Model(
            model_name, adapted_component, adaptor_class, num_steers, rank,
            epsilon, init_var, low_resource_mode)
        return model, model.tokenizer
    else:
